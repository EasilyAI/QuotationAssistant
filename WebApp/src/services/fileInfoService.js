 ...
 * @param {string} fileType - Type of file (FileType enum)
...
const buildFileExistsMessage = (file, fileType) => {
  const fields = {
    ...
    'Year': file.year,
    'Catalog Serial Number': fileType === FileType.Catalog ? file.catalogSerialNumber : null,
    'Ordering Number': fileType === FileType.SalesDrawing ? file.orderingNumber : null,
    'Status': file.status,
...
export const checkFileExistsInS3 = async (formData, fileType) => {
  try {
    ...
    } else if (fileType === FileType.SalesDrawing) {
