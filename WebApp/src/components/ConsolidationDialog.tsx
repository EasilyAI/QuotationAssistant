                ...
                <div className="conflict-side existing">
                  ...
                  <div className="product-preview">
                    <p>
                      ...
                      {conflict.existing.metadata?.catalogProductSnapshot?.description || '—'}
                    </p>
                    <p>
                      <strong>Source:</strong>{' '}
                      {conflict.existing.metadata?.sourceFileName ||
                        conflict.existing.metadata?.sourceFileId ||
                        '—'}
                    </p>
                    {conflict.existing.metadata?.catalogProductSnapshot?.specs &&
                      Object.keys(conflict.existing.metadata.catalogProductSnapshot.specs).length > 0 && (
                      <div className="specs-preview">
                        <strong>Specs:</strong>
                        <ul>
                          {Object.entries(conflict.existing.metadata.catalogProductSnapshot.specs)
                            .slice(0, 3)
                            .map(([key, value]) => (
                              <li key={key}>
                                {key}: {value}
                          ...
                          {Object.keys(conflict.existing.metadata.catalogProductSnapshot.specs).length > 3 && (
                            <li>
                              ... and{' '}
                              {Object.keys(conflict.existing.metadata.catalogProductSnapshot.specs).length - 3} more
                            </li>
                          )}
                        </ul>
                      </div>
